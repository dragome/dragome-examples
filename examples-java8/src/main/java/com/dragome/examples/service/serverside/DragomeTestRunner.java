...
public class DragomeTestRunner extends ParentRunner<FrameworkMethod>
{
	...
	public DragomeTestRunner(Class<?> aClass) throws Exception
	{
		...
		File destinationDirectory= new File("./target/examples-java8");
		...
		page= webClient.getPage(new File(destinationDirectory, "timer-demo.html").toURI().toURL());
	}
	...
	protected void runChild(FrameworkMethod child, RunNotifier notifier)
	{
		...
		String sourceCode= "try{new " + javaToDragomeNotation(clazz) + "()." + javaMethodNameToDragomeNotation(child.getName()) + "(); window.failed=false}catch(e){window.failed=true}";
